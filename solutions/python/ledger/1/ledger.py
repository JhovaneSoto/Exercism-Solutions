...
class LedgerEntry:
    ...
    def __repr__(self):
        return f"LedgerEntry({self.date},{self.description},{self.change})"
...
def format_entries(currency, locale, entries):
    if locale == 'en_US':
        ...
        while len(entries) > 0:
            ...
            entry=sorted(entries,key=lambda x:(x.date,x.change,x.description))[0]
            entries.remove(entry)

            # Write entry date to table
            date_str= f"{entry.date.month:02}/{entry.date.day:02}/{entry.date.year:04}"
            table += date_str
            table += ' | '
